...
function startCommitJob(payload, project, emitter, done) {
  ...
  var cleanup = function () {emitter.removeListener('job.done', function bindOnPhaseHook (data) {onPhaseDoneHook(client, emitter, jobInfo, data)})};
...
function bindOnPhaseHook (data) {
  onPhaseDoneHook(client, emitter, jobInfo, data);
}

function onPhaseDoneHook(client, emitter, jobInfo, data) {
  var urlTtoComment = 'https://api.bitbucket.org/1.0/repositories/' + jobInfo.repositoryFullName + '/pullrequests/' + jobInfo.id + '/comments';
  var phase = data.phases;

  if (phase.test.exitCode !== 0) {
    client.post(urlTtoComment, {content: 'test failed' + String(new Date)}, function() {});

  } else if (phase.deploy.exitCode !== 0) {
    client.post(urlTtoComment, {content: 'test ok, build failed' + String(new Date)}, function() {});

  } else if (phase.test.exitCode === 0 && phase.deploy.exitCode === 0) {
    client.post(urlTtoComment, {content: 'test ok, build ok' + String(new Date)}, function() {});
    emitter.removeListener('job.done', bindOnPhaseHook);
  }
}

function resultHandler(client, emitter, jobInfo) {
  emitter.on('job.done', bindOnPhaseHook);
}
//var cleanup = function () {emitter.removeListener('job.done', function (data) {onPhaseDoneHook(client, emitter, data, jobInfo)})}
