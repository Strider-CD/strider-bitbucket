...
function resultCommentor(client, jobInfo, data) {
  ...
  if (jobInfo.trigger === 'pullrequestJob') {
    ...
    if (phase.test.exitCode !== 0) {
      client.post(urlTtoComment, {content:  ':new_moon:' + '   ' + '***' + exitMessage.testFail + '***' + ':-1:' + '   ' + ';' + '   ' + '***' + 'build CANCELED' + '***' + ':exclamation:' + jobInfo.jobUrl + key/*+ '   ' + errorMessage*/}, function(req, res, erorr) {});

    } else if (phase.deploy.exitCode !== 0) {
      client.post(urlTtoComment, {content: ':last_quarter_moon:' + '   ' + '***' + exitMessage.testPass + '***' + ':+1:' + '   ' + ';' + '   ' + '***' + exitMessage.buildFail + '***' + ':-1:' + '   ' + jobInfo.jobUrl  + key/* + '   ' + errorMessage*/}, function(i, data) {});

    } else if (phase.test.exitCode === 0 && phase.deploy.exitCode === 0) {
      client.post(urlTtoComment, {content: ':full_moon:' + '   ' + '***' + exitMessage.testPass + '***' + ':+1:' + '   ' + ';' + '   ' + '***' + exitMessage.buildPass + '***' + ':+1:' + jobInfo.jobUrl  + key}, function() {console.log(arguments)});
    }
  } else if (jobInfo.trigger === 'commitJob') {
    key = 'job/' + data.id || Math.random().toString(36).substr(2, 18);
    urlTtoComment = API2 + 'repositories' + jobInfo.repositoryFullName + 'commit/' + jobInfo.id + '/statuses/build';

    if (phase.test.exitCode !== 0) {
      client.post(urlTtoComment, {key: key, state: 'FAILED', name: '', url: jobInfo.jobUrl + key, description: exitMessage.testFail /*+ '   ' + ';' + '   ' + errorMessage*/}, function() {});

    } else if (phase.deploy.exitCode !== 0) {
      client.post(urlTtoComment, {key: key, state: 'FAILED', name: '', url: jobInfo.jobUrl + key, description: exitMessage.testPass + '   ' + ';' + '   ' + exitMessage.buildFail /*+ '  ' + errorMessage*/}, function() {});

    } else if (phase.test.exitCode === 0 && phase.deploy.exitCode === 0) {
      client.post(urlTtoComment, {key: key, state: 'SUCCESSFUL', name: '', url: jobInfo.jobUrl + key, description: exitMessage.testPass + '   ' + ';' + '   ' + exitMessage.buildPass}, function() {});
    }
  }
}
