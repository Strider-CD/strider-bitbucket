...
module.exports = {
  ...
  routes: function (app, context) {
    ...
    app.anon.post('pull-request/:secret', function (req, res) {
      ...
      //TODO make pullrequestupdat support
      ...
      if (req.body.pullrequest_updated) {
        // TODO rename pullrequestStatus
        pullrequestStatus = req.body.pullrequest_updated;
        repositoryFullName = pullrequestStatus.source.repository.full_name;

        var hash = pullrequestStatus.source.commit.hash;
        ...
        client.get('https://api.bitbucket.org/2.0/repositories/' + repositoryFullName + '/pullrequests?state=OPEN', function () {
          var res = arguments[1];
          var id = res.values[0].id;
          jobInfo = {id: String(id), repositoryFullName: String(repositoryFullName)};

          api.resultHandler(client, context.emitter, jobInfo);
        })

      } else if (req.body.pullrequest_created) {
		pullrequestStatus = req.body.pullrequest_created;
        repositoryFullName = pullrequestStatus.source.repository.full_name;
        jobInfo = {id: String(pullrequestStatus.id), repositoryFullName: String(pullrequestStatus.source.repository.full_name)};
