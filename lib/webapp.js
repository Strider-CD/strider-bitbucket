...
module.exports = {
  ...
  routes: function (app, context) {
    ...
    app.anon.post('commit/:secret', function (req, res) {
      ...
      // TODO commit status update
      // TODO make jobInfo object

      if (!data.commits[0]) {
        console.log('Got \'empty\' commit')
        return;
      }
      var client = self.oauth(req.accountConfig());
      var id = data.commits[data.commits.length - 1].raw_node;
      var repositoryFullName = data.repository.absolute_url;
      var jobUrl = 'http://' + req.headers.host + repositoryFullName;
      var jobInfo = {id: id, repositoryFullName: repositoryFullName, jobUrl: jobUrl, trigger: 'commitJob'};
    ...
    app.anon.post('pull-request/:secret', function (req, res) {
      ...
      if (!req.body.pullrequest_created && !req.body.pullrequest_updated) {
        return;
      }
      ...
      if (req.body.pullrequest_updated) {
        pullrequest = req.body.pullrequest_updated;
        repositoryFullName = pullrequest.source.repository.full_name;
        jobUrl = 'http://' + req.headers.host + '/' + repositoryFullName + '/';
        var hash = pullrequest.source.commit.hash;
        var isCommitExist = false;

        lastCommits = lastCommits.filter(function(item, i, arr) {
          if (item === hash) {
            isCommitExist = true;
            return false;
          } else {
        ...
        if (isCommitExist) {
          ...
          lastCommits.push(hash);
        }

        client.get(API2 + 'repositories/' + repositoryFullName + '/pullrequests?state=OPEN', function () {
          var res = arguments[1];
          var id = res.values[0].id;

          jobInfo = {id: id, repositoryFullName: repositoryFullName, jobUrl: jobUrl, trigger: 'pullrequestJob'};

          api.resultHandler(client, context.emitter, jobInfo);
        })

      } else if (req.body.pullrequest_created) {
		pullrequest = req.body.pullrequest_created;
        repositoryFullName = pullrequest.source.repository.full_name;
        jobUrl = 'http://' + req.headers.host + '/' + repositoryFullName + '/';
        jobInfo = {id: String(pullrequest.id), repositoryFullName: String(pullrequest.source.repository.full_name), jobUrl: jobUrl, trigger: 'pullrequestJob'};
